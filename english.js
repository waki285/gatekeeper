...
//Discord
client.on('ready', () => {
  logger.success(`Log in with ${client.user.tag}!`);
});
client.on('messageCreate', async message => {
  ...
  if (command === "verify") {
    if (!config['bot_owner_id'].includes(message.author.id)) return logger.warn("Someone other than the bot owner executed this command.");
...
client.on('interactionCreate', async interaction => {
  if (interaction.type === discord.InteractionType.MessageComponent  && interaction.customId === "verify_ja") {
    ...
    .catch(async () => {
      ...
      interaction.followUp(`I couldn't send a message to <@!${interaction.user.tag}>. Instead there is a link here.\nhttps://${config['domain']}/verify/${linkID}`)
      return;
    })
  }
});
